...
class RMD320:
    ...
    def rmd320_update(self,data,ilen):
        if ilen <= 0:
            return None
        ...
        if left != 0 and ilen >= fill:
            ...
            # print('buf=',self.buf)
            self.rmd320_compress(self.buf)
            used += fill
            ilen -= fill
            left = 0

        while ilen >= 64 :
            self.rmd320_compress(data[used:used+64])
            used += 64
            ilen -= 64
    ...
    def rmd320_finish(self):
        ...
        if last < 56:
            ...
            padn = 120 - last
        # 处理填充字段
        self.rmd320_update(padding[0:padn], padn)
        # 处理8字节长度字段
        self.rmd320_update(msglen, 8)
        c = []
        for i in range(10):
    ...
    def rmd320_hash(self, data, ilen):
       self.rmd320_update(data, ilen)
       hash = self.rmd320_finish()
       return hash
...
if __name__ == '__main__':
    ...
    print(data,len(data))
    a = RMD320()
    hash =a.rmd320_hash(data, len(data))
    print("hash:", end='')
    print(byte2hex(hash))
    print('-----------------------')
    str = 'a'
